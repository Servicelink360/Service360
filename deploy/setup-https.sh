...
WWW_DOMAIN="${WWW_DOMAIN:-www.service360.com.au}"
ROOT_DOMAIN="${ROOT_DOMAIN:-service360.com.au}"
...
# All hostnames that serve the admin React app
ADMIN_SERVER_NAMES="${ADMIN_DOMAIN} ${WWW_DOMAIN} ${ROOT_DOMAIN}"

echo "=== Service360 HTTPS setup ==="
echo "API domain:    $API_DOMAIN"
echo "Admin domains: $ADMIN_SERVER_NAMES"
echo ""
...
if [ -n "$PUBLIC_IP" ]; then
  ...
  echo "DNS A records for api, admin, www, and root must point here before certbot runs."
  echo ""
fi
...
install_http_site() {
  local name="$1" server_names="$2" upstream="$3"
...
install_http_site service360-admin "$ADMIN_SERVER_NAMES" 8080
...
install_ssl_site() {
  local name="$1" server_names="$2" upstream="$3" extra="${4:-}"
  cat > "/etc/nginx/sites-available/${name}" <<EOF
server {
    listen 80;
    server_name ${server_names};
...
install_ssl_site service360-admin "$ADMIN_SERVER_NAMES" 8080 ""
...
echo ""
echo "=== Done ==="
echo "Admin: https://${WWW_DOMAIN}/  (also https://${ADMIN_DOMAIN}/)"
echo "API:   https://${API_DOMAIN}/"
echo ""
echo "Open EC2 security group: allow inbound TCP 443 (HTTPS)."
