/**
 * Generate PDF for Adam sample incident (user_tasks id 666) and set pdf_file.
 * Run from service_link_api-main:
 *   npx ts-node -r tsconfig-paths/register scripts/generate-adam-incident-pdf.ts
 */
import { Client } from 'pg';

async function main() {
  // Lazy-load after env so S3 / paths pick up .env
  ...
  if (!resolvedTaskId) {
    // Prefer latest Adam sample incident if no id passed.
    const latest = await client.query(
      `SELECT id FROM user_tasks
       WHERE type = 'CUSTOM' AND report_template_id = 82 AND staff_id = 140
         AND task_name ILIKE 'Incident Report - sample (Adam)%'
       ORDER BY id DESC LIMIT 1`,
    );
    if (!latest.rows.length) throw new Error('No Adam sample incident found. Pass TASK_ID.');
  ...
  if (!task) {
    throw new Error(`user_tasks ${resolvedTaskId} not found`);
  }
  ...
  const tpl = await client.query(
    `SELECT id, name FROM report_templates WHERE id = $1`,
  ...
  const row: any = {
    ...
    reportTemplate: {
      name: tpl.rows[0]?.name || 'Incident Report',
