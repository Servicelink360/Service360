...
const CustomerPersonnelPage: React.FC = () => {
    ...
    try {
      if (editing) {
        const res = await callAPIAsync(
          serviceType.COMMON,
          `${endPoint.CUSTOMER_PERSONNEL}/${editing.id}`,
          'PATCH',
          values,
        );
        if (res?.code !== 1) {
        ...
        const res = await callAPIAsync(serviceType.COMMON, endPoint.CUSTOMER_PERSONNEL, 'POST', values);
        if (res?.code !== 1) {
  ...
  const onRemove = (row: PersonnelRow) => {
    Modal.confirm({
      ...
      content: 'They will no longer appear in delegation lists.',
      okText: 'Remove',
      okButtonProps: { danger: true },
      onOk: async () => {
        await callAPIAsync(
          serviceType.COMMON,
          `${endPoint.CUSTOMER_PERSONNEL}/${row.id}`,
          'DELETE',
          {},
        );
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
          <div>
            ...
            <p style={{ margin: '4px 0 0', color: '#595959' }}>
              Add contacts with custom types (e.g. Tradesperson, Electrician, Support). Fault reports can be delegated by email with a secure link.
            </p>
          </div>
...
export default CustomerPersonnelPage;
