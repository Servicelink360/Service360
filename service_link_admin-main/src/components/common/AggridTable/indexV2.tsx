...
let autoSizeSTO: any
const OrderStatusTable = forwardRef((props: Props, ref: any) => {
  const {
    columnDefs,
    data,
    heightTable,
  ...
  const autoSizeColumn = () => {
    ...
    if (moveRowIndex) {
      ref?.current?.api?.forEachNode(function (node) {
        if (node.rowIndex === moveRowIndex) ref?.current.api!.ensureIndexVisible(node.rowIndex, 'middle')
      })
    }
  }
  ...
  const getRowId = useCallback(
    (params: GetRowIdParams) => {
      ...
      if (rowKey && rowKey === 'CostPaymentNo') return params?.data?.CostPaymentNo
      else return params?.data?.OrderId
    },
    [rowKey]
  )
  ...
  const onCellDoubleClickedv1 = (params: any) => {
    ...
    if (formatter) {
      ...
      if (moment(params.value).isValid() && isNaN(params.value) && cellRenderer) {
        // Check value is date
  ...
  return (
    <div className="ag-theme-balham" style={{ height: heightTable }}>
      <AgGridReact
        ref={ref}
        rowData={data}
        ...
        rowHeight={32}
        loadingOverlayComponent={disableLoading ? null : loadingOverlayComponent}
