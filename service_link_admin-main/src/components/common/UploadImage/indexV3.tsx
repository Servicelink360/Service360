/* eslint-disable no-mixed-operators */
import { Upload, Image, Avatar, Popconfirm } from 'antd'
import { PlusOutlined } from '@ant-design/icons'
import React, { memo, useCallback, useState, useRef, useEffect } from 'react'
import { useIntl } from 'react-intl';
import helperFunc from '@app/lib/helpers/helperFunc'
import { callAPIAsync } from '@app/lib/helpers/api'
import serviceType from '@app/constants/serviceType'
interface UploadImageProps {
    objUserUpdate: any
    onChange: (data: any) => void
}
const UploadImage: any = ({
    onChange,
    objUserUpdate,
    classImage,
}) => {
    const intl = useIntl();
    const [fileList, setFileList] = useState<any[]>([])
    const [imageEmpty, setImageEmpty] = useState<string>('')
    const [imageApi, setImageApi] = useState<string>(objUserUpdate || '')
    const { getBase64_Params } = helperFunc
    const [objUrl, setObjUrl] = useState<any>()
    const refUrl: any = useRef(null)
    const [visiblePreview, setVisiblePreview] = useState<boolean>(false)


    //action upload Image
    const resetImage = (): void => {
        setFileList([])
        setImageApi('');
        onChange('')
    }

    const handleRemoveImageUpdate = useCallback(() => {
        if (imageApi !== '') {
            resetImage()
        } else {
            setFileList([])
            onChange('')
            setImageEmpty('default')
        }
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [imageApi])

    const handleAPI = async (obj: any) => {
        const photo = await getBase64_Params(obj)
        try {
            const response: any = await callAPIAsync(
                serviceType.COMMON, 'v1/users/uploadImage',
                'POST',
                { Image: photo },
            )
            if (response?.message.toLowerCase() === 'success') {
                setImageApi(response?.data)
                onChange(response?.data)
                setImageEmpty('')
            }
        } catch (error) { }
    }

    useEffect(() => {
        if (objUrl && refUrl?.current !== null && refUrl !== objUrl) {
            handleAPI(objUrl)
        }
        refUrl.current = objUrl
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [objUrl])

    const uploadImageAPI = useCallback(
        (obj: any) => {
            setObjUrl(obj)
        },
        // eslint-disable-next-line react-hooks/exhaustive-deps
        [objUrl],
    )

    const handleChange = useCallback(
        ({ fileList }: any) => {
            setFileList(fileList)
            uploadImageAPI(fileList[fileList.length - 1]?.originFileObj)
        },
        // eslint-disable-next-line react-hooks/exhaustive-deps
        [fileList],
    )
    ////end of action upload image


    const uploadButton = (
        <div>
            <PlusOutlined />
            <div style={{ marginTop: 8 }}>{intl.formatMessage({ id: "sidebar.users.upload" })}</div>
        </div>
    )
    const handleRemoveImage = useCallback(() => {
        if (objUserUpdate !== '') {
            handleRemoveImageUpdate()
        } else {
            resetImage()
        }
        setVisiblePreview(false)
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [objUserUpdate])
    return (
        <div style={{ display: 'flex' }}>
            {!objUserUpdate || objUserUpdate === '' || fileList && fileList.length > 0 ? (
                ''
            ) : imageEmpty === '' ? (
                <Image
                    preview={{
                        getContainer: classImage? classImage : '.ant-modal-body',
                        onVisibleChange: (visible, prevVisible) => {
                            setVisiblePreview(visible)
                        },
                    }}
                    style={{ width: '6.5rem', height: '6.5rem', objectFit: 'contain' }}
                    src={objUserUpdate}
                />
            ) : (
                ''
            )}
            {!objUserUpdate || objUserUpdate === '' ? (
                ''
            ) : (
                <div style={{ marginLeft: '0.5rem' }}></div>
            )}

            <Upload
                beforeUpload={() => false}
                listType="picture-card"
                fileList={fileList}
                /* @ts-ignore */
                maxCount={1}
                itemRender={(originNode: any, file: any) => {
                    // uploadImage(file?.originFileObj)
                    return (
                        <div>
                            {file.type.indexOf('image') !== -1 ? (
                                <Image
                                    preview={{
                                        getContainer: classImage? classImage : '.ant-modal-body',
                                        onVisibleChange: (visible, prevVisible) => {
                                            setVisiblePreview(visible)
                                        },
                                    }}
                                    src={file?.thumbUrl}
                                />
                            ) : (
                                ''
                            )}
                        </div>
                    )
                }}
                showUploadList={{ showRemoveIcon: true }}
                onChange={handleChange}
            >
                {fileList.length >= 8 ? null : uploadButton}
            </Upload>
            {/* Trash For Images */}
            {visiblePreview ? (
                <Popconfirm
                    title={intl.formatMessage({ id: "sidebar.users.confirm" })}
                    okText={intl.formatMessage({ id: 'button.Yes' })}
                    cancelText={intl.formatMessage({ id: 'button.No' })}
                    placement="topRight"
                    onConfirm={(e) => {
                        handleRemoveImage()
                    }}
                >
                    <div
                        style={{
                            position: 'fixed',
                            right: '7.25%',
                            bottom: '13.5%',
                            zIndex: 9999,
                        }}
                    >
                        <Avatar
                            style={{
                                backgroundColor: '#ffffff',
                                cursor: 'pointer',
                                width: '2.75rem',
                                height: '2.75rem',
                                display: 'flex',
                                justifyContent: 'center',
                                alignItems: 'center',
                            }}
                        >
                            <i
                                style={{
                                    color: 'rgb(227,85,76)',
                                    marginTop: '0.5rem',
                                    fontSize: '1.25rem',
                                }}
                                className="ion-android-delete"
                            />
                        </Avatar>
                    </div>
                </Popconfirm>
            ) : null}
        </div>
    )
}
export default memo(UploadImage)
