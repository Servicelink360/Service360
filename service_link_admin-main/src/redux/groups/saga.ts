...
function* getData() {
    yield takeEvery(actions.GET_DATA, function* ({ payload }: any) {
        try {
            const response: ListResponse<any> = yield callAPI(serviceType.COMMON, `${endPoint.GROUPS}`, "GET", payload);
            if (response?.code === errorCode.SUCCESS) {
                yield put({
...
function* saveInto({ payload }: any) {
    try {
        ...
        switch (actionName) {
            case actionType.ADD:
                response = yield callAPI(serviceType.COMMON, endPoint.GROUPS, method.POST, data);
                break;
            case actionType.DELETE:
                response = yield callAPI(serviceType.COMMON, `${endPoint.GROUPS}/${data.id}`, method.DELETE, {});
                break;
            case actionType.UPDATE:
                response = yield callAPI(serviceType.COMMON, `${endPoint.GROUPS}/${data.id}`, method.PATCH, data);
                break;
            default:
                break;
        }
