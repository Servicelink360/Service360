...
function* saveInto({ payload }: any) {
    try {
        ...
        let response: ListResponse<any> = { data: null, code: null, message: '' }
        switch (actionName) {
            case actionType.ADD:
                response = yield callAPI(serviceType.COMMON, endPoint.COMPANIES, method.POST, data)
                break
...
export default function* rootSaga() {
    yield all([fork(getData), takeEvery(actions.SAVE_INTO, saveInto)])
}
