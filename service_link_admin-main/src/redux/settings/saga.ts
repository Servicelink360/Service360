...
function* getData() {
    yield takeEvery(actions.GET_DATA, function* ({ payload }: any) {
        try {
            const response: ListResponse<any> = yield callAPI(serviceType.COMMON, `${endPoint.SETTINGS}`, "GET", payload);
    ...
    try {
        ...
        switch (actionName) {
            case actionType.UPDATE:
                response = yield callAPI(serviceType.COMMON, endPoint.SETTINGS_UPDATE_BULK, method.POST, data);
                break;
            default:
                break;
        }
