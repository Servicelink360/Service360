...
function* getDataInit() {
    yield takeEvery(actions.GET_DATA_INIT, function* ({ payload }: any) {
        try {
            const response: ListResponse<any> = yield callAPI(serviceType.COMMON, `${endPoint.COMMON}/getInitData?items=SERVICES`, "GET", payload);
...
function* getData() {
    yield takeEvery(actions.GET_DATA, function* ({ payload }: any) {
        try {
            const response: ListResponse<any> = yield callAPI(serviceType.COMMON, `${endPoint.POSITIONS}`, "GET", payload);
            if (response?.code === errorCode.SUCCESS) {
                yield put({
...
function* saveInto({ payload }: any) {
    try {
        ...
        switch (actionName) {
            case actionType.ADD:
                response = yield callAPI(serviceType.COMMON, endPoint.POSITIONS, method.POST, data);
                break;
            case actionType.DELETE:
                response = yield callAPI(serviceType.COMMON, `${endPoint.POSITIONS}/${data.id}`, method.DELETE, {});
                break;
            case actionType.UPDATE:
                response = yield callAPI(serviceType.COMMON, `${endPoint.POSITIONS}/${data.id}`, method.PATCH, data);
                break;
            default:
                break;
        }
